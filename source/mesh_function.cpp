...
ma_real& MeshFunction::operator[](int i){
    return this->values[i];
}

void MeshFunction::init(ma_real (*f)(Point)) {
    for (int i = 0; i < this->mesh.num_points; i++) {
        this->values[i] = f(this->mesh.points[i]);
    }
}

ma_real MeshFunction::interpolate(Point p) {
...
ma_real MeshFunction::interpolate(Point p, int id) {
    // Locate element
    Element e = this->mesh.elements[id];
    Point v0 = this->mesh.points[e.v[0]];
    Point v1 = this->mesh.points[e.v[1]];
...
ma_real MeshFunction::second_difference(int id, Point v, ma_real delta) {
    ma_real rho = 1;
    ...
    ma_real diff = (this->interpolate(p_plus) - 2 * this->values[id] +
        this->interpolate(p_minus)) / (rho * rho * delta * delta);
    return diff;
}

ma_real MeshFunction::second_difference(int id, Point p_plus, int e_plus_id,
                                     Point p_minus, int e_minus_id, ma_real rho, ma_real delta) {
    ma_real diff = (this->interpolate(p_plus, e_plus_id) - 2 * this->values[id] +
        this->interpolate(p_minus, e_minus_id)) / (rho * rho * delta * delta);
    return diff;
}

ma_real MeshFunction::norm_inf() {
    ma_real ret = 0;
    for (int i = 0; i < (int)values.size(); i++) {
